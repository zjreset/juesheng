...
@synthesize slaveArray = _slaveArray;
...
- (void) getPhotoInfo
{
    ...
    [request send];
    request.userInfo = [NSNumber numberWithInt:1];
...
- (void)requestDidFinishLoad:(TTURLRequest*)request {
    ...
    if (!success) {
        ...
        if ([(NSNumber*)request.userInfo integerValue] == 1) {
            NSMutableArray *photoArray = [[NSMutableArray alloc] init];
            _slaveArray = [[Slave alloc] initWithDict:[resultJSON objectForKey:@"classSlaveList"]];
            MockPhoto *mockPhoto;
            [[TTURLRequestQueue mainQueue] setMaxContentLength:0];
            for (Slave *slave in _slaveArray){
                ...
                mockPhoto = [[[MockPhoto alloc] initWithURL:_filePath smallURL:_filePath size:CGSizeMake(0, 0) caption:[NSString stringWithFormat:@"名称:%@  大小:%@B\n上传日期:%@",slave.fFileName,slave.fSize,slave.fDate]] autorelease];
            ...
            if (photoArray.count > 0) {
                UIBarButtonItem *viewItem = [[[UIBarButtonItem alloc] initWithTitle:TTLocalizedString(@"查看全部",@"查看全部缩略图") style:UIBarButtonItemStyleBordered target:self action:@selector(showThumbnails)] autorelease];
                UIBarButtonItem *deleteItem = [[[UIBarButtonItem alloc] initWithTitle:@"删除" style:UIBarButtonItemStyleBordered target:self action:@selector(deleteEntry)] autorelease];
                NSArray *rightArray = [NSArray arrayWithObjects:viewItem,deleteItem, nil];
                self.navigationItem.rightBarButtonItems = rightArray;
            }
            [photoArray release];
        }
        else if ([(NSNumber*)request.userInfo integerValue] == 2) {
            UIAlertView * alert= [[UIAlertView alloc] initWithTitle:@"删除图片成功!" message:nil delegate:self cancelButtonTitle:@"确定" otherButtonTitles: nil];
...
- (void)deleteEntry{
    Slave *slave = [_slaveArray objectAtIndex:_centerPhotoIndex];
    if (slave) {
        AppDelegate *delegate=(AppDelegate*)[[UIApplication sharedApplication] delegate];
        NSString *server_base = [NSString stringWithFormat:@"%@/slave!deleteClassSlave.action", delegate.SERVER_HOST];
        
        TTURLRequest* request = [TTURLRequest requestWithURL: server_base delegate: self];
        [request setHttpMethod:@"POST"];
        
        request.contentType=@"application/x-www-form-urlencoded";
        NSString* postBodyString = [NSString stringWithFormat:@"isMobile=true&classType=%i&fItemId=%i&fEntryId=%@",_classType,_fItemId,slave.fEntryId];
        request.cachePolicy = TTURLRequestCachePolicyNoCache;
        NSData* postData = [NSData dataWithBytes:[postBodyString UTF8String] length:[postBodyString length]];
        ...
        [request send];
        request.userInfo = [NSNumber numberWithInt:2];
