...
+(void)buttonSelected:(RadioButton*)radioButton{
    ...
    if (rb_observers) {
        ...
        if(observer && [observer respondsToSelector:@selector(radioButtonSelectedAtIndex:inGroup: inValue:)]){
            [observer radioButtonSelectedAtIndex:radioButton.index inGroup:radioButton.groupId inValue:radioButton.groupValue];
        }
    }
    ...
    if (rb_instances) {
        for (int i = 0; i < [rb_instances count]; i++) {
            ...
            if (![button isEqual:radioButton] && [button.groupId compare:radioButton.groupId] == NSOrderedSame) {
